...
public class SceneTransitionManager : MonoBehaviour
{
    ...
    private bool m_DisableCharacterForTime = false;
    private float m_DisabledCharacterTime;
    ...
    void Update()
    {
        if (m_DisableCharacterForTime)
        {
            m_DisabledCharacterTime -= Time.deltaTime;
            if (m_DisabledCharacterTime <= 0)
            {
                m_Player.GetComponent<StarterAssets.FirstPersonController>().enabled = true;
                m_DisableCharacterForTime = false;
            }
        }
    ...
    public static void Teleport()
    {
        ...
        instance.m_DisableCharacterForTime = true;
        instance.m_DisabledCharacterTime = 0.2f;
